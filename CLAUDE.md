...
| `web/assets/install.sh` | the one-line installer, served at `/install.sh`: latest release's .dmg, SHA-256 checked against GitHub's digest, into /Applications, quarantine cleared. Test with `MENU_OTP_INSTALL_DIR=<tmp> MENU_OTP_NO_OPEN=1 sh web/assets/install.sh`; it only quits a running copy at the path it replaces |
...
- `.github/workflows/deploy-web.yml` builds it on Node 24 and deploys `web/dist` to
  ...
  served from the custom domain `otp.iambrian.com` (set in the repo's Pages settings
